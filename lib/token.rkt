...
(define callback "") ;not needed for single-user authentication
...
(define (generate-oauth-auth-string request-url)
...
(define the-port
  (get-pure-port
    (string->url request-url)
      (list 
      (generate-oauth-auth-string request-url))))

(generate-base-string "get" "https://api.twitter.com/1/statuses/home_timeline.json?include=true" 
                      (url-query 
                       (string->url "https://api.twitter.com/1/statuses/home_timeline.json?include=true")) )

 
(define (get-base-url full-url)
  
  (connect-path (map path/param-path (url-path full-url))
  )
